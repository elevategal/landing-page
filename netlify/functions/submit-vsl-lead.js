...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { name, phone, email, consent, vslPercent, utmSource, utmMedium, utmCampaign, utmContent,
    ...
    const fields = {
      ...
      'אחוז צפיית VSL': safeVslPct
    };

    let response = await fetch(`https://api.airtable.com/v0/${BASE_ID}/${encodeURIComponent(TABLE_NAME)}`, {
      method: 'POST',
      headers: {
    ...
    if (result.error && result.error.type === 'UNKNOWN_FIELD_NAME') {
      delete fields['אחוז צפיית VSL'];
      response = await fetch(`https://api.airtable.com/v0/${BASE_ID}/${encodeURIComponent(TABLE_NAME)}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${AIRTABLE_TOKEN}`,
          'Content-Type': 'application/json'
      ...
      console.warn('VSL field missing in Airtable — retried without it. Add column "אחוז צפיית VSL" (Number).');
    }
